"""빌더 self-check (지식베이스 §6-1) — 계획고·bbox·내공 H 8점·부재 개수·받침 EL·수밀.

결과는 PASS/FAIL 목록(JSON)으로 남긴다. pilot(본체·격벽만) 이면 개수·받침 항목은 skipped.
"""
...
def run(named: dict[str, trimesh.Trimesh], b: Builder, *, pilot: bool = False) -> dict:
    s = b.s
    checks: list[dict] = []
    ...
    def skip(label):
        checks.append({"label": label, "ok": None, "detail": "pilot — skipped"})

    # 0) 계획고 기준 검산 — EL(P4)/EL(P5) 는 계획고 식에서 유도(정보 + 단조 증가)
    el4, el5 = b.el_road(b.Z_P4), b.el_road(b.Z_P5)
    check("계획고 P4<P5(종단경사 부호)", el5 > el4, "EL(P4)=%.3f EL(P5)=%.3f" % (el4, el5))
    ...
    check("bbox x(상부구조)", abs(bb[0][0] + x_exp) < 0.01 and abs(bb[1][0] - x_exp) < 0.01,
          "x[%.3f, %.3f] (기대 ±%.2f)" % (bb[0][0], bb[1][0], x_exp))
    ...
    check("bbox z(상부구조)", abs(bb[0][2] - z_lo_exp) < 0.05 and abs(bb[1][2] - z_hi_exp) < 0.16,
          "z[%.3f, %.3f] (기대 [%.2f, %.2f])" % (bb[0][2], bb[1][2], z_lo_exp, z_hi_exp))
    ...
    for k, name in enumerate(dia):
        ...
        if np.min(np.abs(zs - zc)) > 0.01 or zs.min() < zc - 0.4 or zs.max() > zc + 0.4:
            chain_ok = False
    check("격벽 z 체인 P4+%.1fk" % s.diaphragm.spacing, chain_ok, "판면 정점이 체인 위치 ±10mm")

    def cnt(pat):
        return sum(1 for n in named if re.match(pat, n))
    if pilot:
        for label in ("프레임 부재 25×6=150", "WG 부재 그룹 52", "CS 52", "받침 부품 16", "받침 하면 y"):
            skip(label)
    else:
        n_frm = cnt(r"^AB1_S5_FRM\d{2}_(TRW|TRF|BRW|BRF|VSL|VSR)$")
        check("프레임 부재 25×6=150", n_frm == 150, "%d" % n_frm)
        n_wg = cnt(r"^AB1_S5_WG\d{3}[LR]$")
        check("WG 부재 그룹 52", n_wg == 52, "%d" % n_wg)
        n_cs = cnt(r"^AB1_S5_CS\d{3}[LR]$")
        check("CS 52", n_cs == 52, "%d" % n_cs)
        n_brg = cnt(r"^AB1_S5_BRG_P[45]_[12]_(SOLE|BODY|MORTAR|BLOCK)$")
        check("받침 부품 16", n_brg == 16, "%d" % n_brg)
        for pier, el in s.bearing.el_check.items():
            ...
            for i in (1, 2):
                ...
                check("받침 하면 y %s_%d" % (pier, i), m is not None and abs(y_meas - y_exp) < 0.01,
                      "실측 %.4f (기대 %.3f)" % (y_meas, y_exp))

    # 4) 메시 수·수밀·색상
    n_mesh = len(named)
    lo, hi = (25, 40) if pilot else (500, 700)
    check("메시 수 %d~%d" % (lo, hi), lo <= n_mesh <= hi, "%d" % n_mesh)
    n_wt = sum(1 for m in named.values() if m.is_watertight)
    n_col = sum(1 for m in named.values()
                if np.asarray(m.visual.face_colors).shape[0] == len(m.faces))
    check("버텍스 컬러 전건", n_col == n_mesh, "%d/%d" % (n_col, n_mesh))
    ...
    return {"checks": checks, "pass": passed, "fail": failed, "skipped": skipped,
            "meshes": n_mesh, "watertight": n_wt, "triangles": tris, "pilot": pilot}
