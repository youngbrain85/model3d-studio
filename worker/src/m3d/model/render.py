...
def view_contract(frame: dict, center) -> dict:
    ...
    P = origin + u·extent[0]·u_axis + v·extent[1]·v_axis (u,v ∈ [0,1] 이 축 범위 xlim/ylim 에 대응).
    """
...
def run_render(glb: Path, out_dir: Path, spec: ModelSpec, *, pilot: bool = False) -> list[Path]:
    ...
    def shot(V, F, C, eye, name, title, **kw):
        """렌더 1장 + 뷰 계약 수집."""
        fr: dict = {}
        written.append(render(V, F, C, eye, Y_UP, out_dir / (name + ".png"), title, frame_out=fr, **kw))
        views[name] = {"file": name + ".png", "title": title, **view_contract(fr, V.mean(axis=0))}
    ...
    shot(V, F, C, (1, 0, 0), "side_context",
         "측면 맥락 — P4~P5 %.0fm 변단면(인접 경간 스텁·교각 개략)" % spec.coord.span, size=(20, 5), extra=context)
    ...
    V, F, C = to_tris(half)
    shot(V, F, C, (0, 0, 1), "front_section",
         "정면 단면(z %.1f~%.1f 절편, +z 측에서, 좌=보도측) — 격벽·개구·WG·CS·방호벽" % (z_lo, z_hi), size=(12, 7))
    if pilot:
        return _write_views(out_dir, views, written)

    # 3) 저면 아이소 — 카메라가 아래(−y)·+x·+z 방향: 하판·받침·스트럿·CS 가 보인다
    V, F, C = to_tris(nodes, max_edge=1.0)
    shot(V, F, C, (0.55, -1.0, 0.4), "bottom_iso",
    ...
    shot(V, F, C, (0.7, 0.45, 0.85), "interior_cells",
         "내부 격실 컷(x<0 절개·상판 제거, P4+10~+30) — 격벽·개구·리브·프레임·수평보강재", size=(14, 8), edge_alpha=0.0)
    return _write_views(out_dir, views, written)
