...
| 형태 | 수량 | 용량 | 파이프라인 위치 |
|---|---|---|---|
| DXF 원본 (`_dxf/`) | 50 | 262 MB | **[1] 입력 — 벡터 정본** |
| 통합 PDF (`_정밀조사패키지_P4P5/`) | 1 (61p) | 47 MB | **[1] 입력 — 래스터 경로** |
| PNG 원본해상도 (동상) | 61 | 151 MB | **[2] 변환 산출물 = 회귀 정답** |
| **합계** | **112** | **약 460 MB** | |
...
| D2 | **샘플은 로컬 복사** (`data/samples/`, gitignore) | 참조 원본 수정 위험이 구조적으로 0. 460MB 디스크 중복은 수용 |
...
M0의 로직다운 로직은 파서·매니페스트뿐이다. 여기에만 단위테스트를 쓴다.
...
DB·복사·CLI 통합 동작은 §9 표의 실행으로 검증한다(별도 통합테스트를 M0에 만들지 않는다).
...
| 460MB 복사 중단 | `collect`는 재실행 가능(멱등) — 해시 일치 파일은 건너뜀 |
