...
| 8 | `npm run dev` | 헬스 화면 "도달 OK / projects 0행(RLS 차단)" → **화면 증거 캡처**(아래 주 참조) |

5·6(DB 직결 — `postgres` 역할의 BYPASSRLS)과 8(anon으로 0행)을 나란히 두면 RLS가 실제로
막고 있음이 증명된다. 8의 화면 증거는 전역 규칙 §2("검증 시 영상/화면 캡처 대조")의 이행이다.

**8의 화면 증거 개정 (2026-08-29, 사용자 승인):** 이 개발 환경에서는 브라우저 창이
화면에 합성되지 않아 픽셀 스크린샷이 불가함이 3자(구현자·리뷰어·컨트롤러)에 의해
독립 재현되었다. 이에 8의 증거는 **DOM 텍스트 + 접근성 트리 + 실제 REST 호출 캡처**
(요청 URL·HTTP 200·응답 본문 `[]`·worker 행수 대조표)의 번들로 대체한다. 이 번들은
픽셀 캡처보다 정보량이 많다 — 렌더 문구뿐 아니라 실제 네트워크 응답 본문까지 담는다.
픽셀 캡처가 가능한 환경(사용자 인터랙티브 세션)에서는 원문 요건이 그대로 유효하다.
