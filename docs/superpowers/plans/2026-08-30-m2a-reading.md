...
Expected: **177 passed** = 160 + cache 8 + client 9. (test_config 는 함수 수 변화 없음.)
...
Expected: **200 passed** = 177 + schema 15 + inputs 8. 실측 우선.
...
Expected: **208 passed** = 200 + sheet 8. 실측 우선.
...
Expected: 2페이지 처리, readings·ambiguities 개수와 비용 출력. 비용은 **`usage.jsonl` 합계로 확인**한다 — 통상 $0.10~0.20, 재시도 포함 최악 ≈$0.45(입력 ≈$0.04 + 출력 상한 8000tok×2×$10/M). `MAX_TOKENS` 를 임시로 낮추지 않는다(절단 → 검증 실패 → 재시도로 오히려 비용이 2배가 된다).
...
Expected: **221 passed** = 208 + region 7 + store 6. 실측 우선.
...
Expected: **233 passed** = 221 + crops 12.
