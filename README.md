...
실증(요청 없는 잡 3건 · 12시도 · 15호출 · $1.55, 상한 $8): 최대 bbox 편차 326 mm(M5) → **8 mm**(잡 1) → 90 mm(잡 2, 개구보강재 돌출을 두께로 해석) → **5.001 mm**(잡 3, 모든 검사 0 fail·삼각형 수 정답과 동일).
원 규칙(≤ 5.000 mm)으로는 3잡 모두 FAIL 이고, 반올림 여유를 둔 규칙으로 잡 3 최종 시도는 PASS(오프라인 재채점, `model/agent/rescore/`). 화면·DB 의 b8 은 실행 당시 규칙대로 FAIL 로 남겨 두었다.
